...
extern DP::TransactionManager_t& GetTransactionManager();
...
namespace DP {
  ...
  public:
    ...
    static TransactionManager_t& Get();
  ...
  private:
    ...
    Data_t* GetTransactionExecuting();

    static    void CALLBACK    ApcExecute(ULONG_PTR Param);

    static    void CALLBACK    ApcComplete(ULONG_PTR Param);

    void    SendEvent(
      const wchar_t* name,
      Transaction::Data_t* pPrevTxData = nullptr);

    bool    DoExecuteTransaction(
      Transaction::Data_t* pData);

    bool    SetTransactionExecuting(
      Transaction::Data_t* pData,
      bool fInterrupt = false);

    TransactionManager_t::Data_t* FindTransaction(
