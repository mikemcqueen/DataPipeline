...
HRESULT TrWindowType_t::MessageHandler(const DP::Message::Data_t* pMessage) {
...
Ui::WindowId_t TrWindowType_t::CompareAllLastOrigins(const CSurface& surface) const {
  using namespace Ui::Window;
  Ui::WindowId_t windowId = Id::Unknown;
...
Ui::WindowId_t TrWindowType_t::SearchAllWindows(const CSurface& surface) const {
  using namespace Ui::Window;
  Ui::WindowId_t windowId = Id::Unknown;
  for (int func = 0; func < _countof(s_windowIdFuncs); ++func) {
